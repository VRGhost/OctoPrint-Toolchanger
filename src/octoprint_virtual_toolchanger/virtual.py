...
class VirtualPrinter:
    ...
    def __init__(
        ...
        self._logger = logging.getLogger("octoprint_virtual_toolchanger.VirtualPrinter")
        ...
        self._seriallog = logging.getLogger(
            "octoprint_virtual_toolchanger.VirtualPrinter.serial"
        )
        ...
        self._action_hooks = plugin_manager().get_hooks(
            "octoprint_virtual_toolchanger.custom_action"
        )
        ...
        readThread = threading.Thread(
            ...
            name="octoprint_virtual_toolchanger.wait_thread",
        )
        readThread.start()

        bufferThread = threading.Thread(
            target=self._processBuffer,
            name="octoprint_virtual_toolchanger.buffer_thread",
        )
        bufferThread.start()
    ...
    def _processIncoming(self):
        ...
        while self.incoming is not None and not self._killed:
            ...
            if (
                self._resend_every_n
                and self._received_lines % self._resend_every_n == 0
            ):
                self._triggerResend(checksum=True)
                continue
    ...
    def _gcode_M851(self, data: str) -> None:
        ...
        if not self._check_param_letters("XYZ", data):
            ...
            for key, value in self._parse_eeprom_params("XYZ", data).items():
                self._virtual_eeprom.eeprom["probe_offset"]["params"][key] = float(
                    value
                )
    ...
    def _debugTrigger(self, data: str) -> None:
        if data == "" or data == "help" or data == "?":
            ...
            self._prepared_errors.append(
                lambda cur, last, ln: self._triggerResend(
                    expected=last, actual=last + 1
                )
            )
            ...
            try:
                ...
                sleep_after_next_match = VirtualPrinter.sleep_after_next_regex.match(
                    data
                )
    ...
    def _selectSdFile(self, filename: str, check_already_open: bool = False) -> None:
        ...
        if self._settings.get_boolean(["includeFilenameInOpened"]):
            self._send(
                "File opened: %s  Size: %d" % (filename, self._selectedSdFileSize)
            )
        else:
    ...
    def _parseHotendCommand(
        ...
        try:
            ...
            if support_r:
                try:
                    self.targetTemp[tool] = float(
                        re.search(r"R([0-9]+)", line).group(1)
                    )
    ...
    def _parseChamberCommand(self, line, wait=False, support_r=False):
        ...
        try:
            ...
            if support_r:
                try:
                    self.chamberTargetTemp = float(
                        re.search("R([0-9]+)", line).group(1)
                    )
                    only_wait_if_higher = False
                except Exception:
                    pass
    ...
    def _waitForHeatup(self, heater: str, only_wait_if_higher: bool) -> None:
        ...
        try:
            if heater.startswith("tool"):
                ...
                test = lambda: self.temp[toolNum] < self.targetTemp[
                    toolNum
                ] - delta or (
                ...
                test = lambda: self.bedTemp < self.bedTargetTemp - delta or (
                    not only_wait_if_higher
                    and self.bedTemp > self.bedTargetTemp + delta
                )
            ...
            while not self._killed and self._heatingUp and test():
                ...
                if (
                    self._sendBusy
                    and time.monotonic() - last_busy >= self._busyInterval
                ):
